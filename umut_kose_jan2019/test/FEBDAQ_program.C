...
TGStatusBar *fStatusBar739;
   ...
   TGCheckButton *fMonitorOut;
   TGCheckButton *fMonitorIn;
   TGNumberEntry *fMonitorOutValue;
   TGTab *fTab683;
   TGLabel *fLabel7;
...
void FEBDAQ_program(const char *iface="eth1")
{
...
if(!(t->ReadBitStream(bsname,bufSCR))) t->ReadBitStream("CITIROC_SC_PROFILE1_AM.txt",bufSCR);
...
//		printf("%04u ",adc);
	...
	COINC = *(UInt_t*)(&(t->gpkt).Data[jj]); jj=jj+4; 

 printf("T0=%u ns, T1=%u ns T0_ref=%u ns  T1_ref=%u ns  \nCOINC=",ts0,ts1,ts0_ref,ts1_ref);

 print_binary(COINC,16);
...
void EnableDisableOutputMonitor()
{
	UShort_t cfg;
	printf("EnableDisableOutputMonitor() is %d; bypass in is: %d; Monitor on channel %d\n", (int)fMonitorOut->IsOn(), (int)fMonitorIn->IsOn(), (int)fMonitorOutValue->GetNumber());
	cfg = 0;
	cfg = 	((int)fMonitorOut->IsOn() << 0) +
		((int)fMonitorIn->IsOn() << 1) +
		((int)fMonitorOutValue->GetNumber() << 2);
	t->dstmac[5]=0xff; //Broadcast
	t->SendCMD(t->dstmac,FEB_SET_FPGA_CFG,cfg,buf);  
}
...
TGTextButton *fTextButton78, *fTextButton88;
...
//{
//SetDstMacByIndex(feb);
fTextButton78->ChangeBackground(0xbbffbb);
t->dstmac[5]=0xff; //Broadcast
...
//{
//SetDstMacByIndex(feb);
fTextButton78->ChangeBackground(0xffffff);
t->dstmac[5]=0xff; //Broadcast
...
void FEBGUI()
{
   ...
   fGroupFrame679->AddFrame(fNumberEntry755, new TGLayoutHints(kLHintsLeft | kLHintsTop,2,2,22,2));
  ...
  TGTextButton *fTextButton9319 = new TGTextButton(fGroupFrame679,"Set Threshold Moned");
   ...
   fGroupFrame679->AddFrame(fLabel764, new TGLayoutHints(kLHintsLeft | kLHintsTop | kLHintsExpandX,2,2,20,2));
...
*/
   ...
   fGroupFrame679->AddFrame(fNumberEntry75, new TGLayoutHints(kLHintsLeft | kLHintsTop,2,2,22,2));
   ...
   fMonitorOut = new TGCheckButton(fGroupFrame679,"Enable Monitor Output");
   fMonitorOut->SetTextJustify(36);
   fMonitorOut->SetMargins(0,0,0,0);
   fMonitorOut->SetWrapLength(-1);
   fMonitorOut->SetCommand("EnableDisableOutputMonitor()");
   fGroupFrame679->AddFrame(fMonitorOut, new TGLayoutHints(kLHintsLeft | kLHintsTop,2,2,2,2));


   fMonitorIn = new TGCheckButton(fGroupFrame679,"Bypass input");
   fMonitorIn->SetTextJustify(36);
   fMonitorIn->SetMargins(0,0,0,0);
   fMonitorIn->SetWrapLength(-1);
   fMonitorIn->SetCommand("EnableDisableOutputMonitor()");
   fGroupFrame679->AddFrame(fMonitorIn, new TGLayoutHints(kLHintsLeft | kLHintsTop,2,2,2,2));


   fMonitorOutValue = new TGNumberEntry(fGroupFrame679, (Double_t) 1,6,-1,(TGNumberFormat::EStyle) 5,(TGNumberFormat::EAttribute) 1,(TGNumberFormat::ELimit) 2,0,32);
   fGroupFrame679->AddFrame(fMonitorOutValue, new TGLayoutHints(kLHintsLeft | kLHintsTop,2,2,12,2));
   fMonitorOutValue->Connect("ValueSet(Long_t)", 0, 0,  "EnableDisableOutputMonitor()");
   ...
   fGroupFrame679->AddFrame(fLabel7, new TGLayoutHints(kLHintsLeft | kLHintsTop | kLHintsExpandX,2,2,19,2));
  ...
  // fNumberEntry8869->SetCommand("UpdateBoardMonitor()");
   ...
   TGTextButton *fTextButtonEx = new TGTextButton(fGroupFrame679,"&Exit");
   fTextButtonEx->SetTextJustify(36);
   fTextButtonEx->SetMargins(0,0,0,0);
   fTextButtonEx->SetWrapLength(-1);
   fTextButtonEx->Resize(123,22);
   fTextButtonEx->SetCommand("Exit()");
   fGroupFrame679->AddFrame(fTextButtonEx, new TGLayoutHints(kLHintsLeft| kLHintsCenterX  | kLHintsTop | kLHintsExpandX,0,0,10,2));
